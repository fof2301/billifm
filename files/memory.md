...
| D21 | **Checkpoint choices are VISIBLE predefined menus** (2–3 options, shown on screen at cliffhanger moments). Supersedes D15 *for checkpoint moments only* — the invisible-constraint A/B/C schema stays for live in-call conversations (Dev confrontation, lure-resistance). | New demo direction: showcase user-driven branching with predefined options; voice-IF research says 2–3 restated options at cliffhangers | 2026-07-26 |
| D22 | **New canonical story: "Riya Calling"** replaces *Aakhri Awaaz* (files/story.md is dead). 5 episodes, 4 checkpoints/playthrough, exactly 30 paths, 5 endings (default E4 "Wahi Raat"); reconvergent braided structure — plot/motive invariant on every path, fate mechanic covers the rails in-fiction. Source of truth: `story/` (bible, branch-map, episode scripts, story_graph.json). Personas: Zoya (advice agent), Riya (emotional). New effects needed: message_overlay, outgoing_call, checkpoint_menu. | Old story judged unusable; new premise is phone-native (calls/texts ARE the story) and its time-loop fate mechanic justifies reconvergence so users never feel the rails | 2026-07-26 |
...
### 2026-07-26 — Session 3 (story rebuild + branching architecture, with Claude Code)
- Built: `story/` — new canonical story **Riya Calling** (bible, branch-map with 30-path/5-ending architecture + reconvergence proofs, full Hinglish episode scripts ep1–ep5 incl. two Ep4 tracks and five ending scripts, story_graph.json, Pocket FM formula research). Decisions D21 (visible checkpoint menus) + D22 (story replacement) logged.
- Research: Pocket FM formula (8–12 min constant episodes, hook <30s, mid-spike + end cliffhanger every episode, biggest cliffhanger at ep3→4 paywall slot, 2–3 options max at voice choice points) — see story/research.md.
- Engineering deltas identified, not built: message_overlay / outgoing_call / checkpoint_menu effects, zoya.md + riya.md personas, N-way branch audio swap in engine.
- Next: audio gen for showcase excerpts; wire checkpoint_menu into the app; refit server prompts to Zoya/Riya; map story_graph.json into eval sim for persona testing.
